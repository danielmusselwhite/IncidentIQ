...
IncidentIQ therefore complements its automated retrieval and citation checks with a lightweight human-review step.

For each evaluation run, the evaluator writes a Markdown review report alongside the machine-readable JSON report.

The generated review report contains, for every evaluation scenario:

- the evaluation input;
- the expected historical-Incident and Runbook evidence;
- the actual structured response generated by Azure AI;
- automated citation and grounding results;
- a human-review checklist;
- space for reviewer notes.

This makes it possible to inspect the exact response that was evaluated without introducing an additional LLM-as-judge system.

### Human Review Criteria

Generated responses are assessed against:

| Criterion | Question |
| --- | --- |
| Summary quality | Does the response accurately describe the operational problem? |
| Likely-cause relevance | Are proposed causes relevant to the Incident and supplied evidence? |
| Recommended-action relevance | Are recommended actions useful and appropriate for the supplied context? |
...
The review deliberately uses qualitative `Pass`, `Concern`, or `N/A` outcomes rather than artificial numerical scores.

### Representative Human Review

Representative scenarios from the current baseline were manually reviewed:

| Scenario | Summary | Cause relevance | Action relevance | Uncertainty | Grounding |
| --- | --- | --- | --- | --- | --- |
| Payment gateway timeout | Pass | Pass | Pass | Pass | Pass |
| Ambiguous dependency timeout | Pass | Pass | Pass | Pass | Pass |
| No indexed evidence | Pass | N/A | Concern | Pass | Concern |

The evidence-backed scenarios produced relevant operational guidance while remaining appropriately cautious where multiple explanations were possible.

The no-evidence scenario correctly stated that the root cause could not be determined and returned no evidence references. However, it also produced generic Kafka diagnostic and mitigation guidance despite receiving no retrieved evidence.

This is recorded as a known grounding limitation rather than being hidden from the evaluation results.

It demonstrates an important distinction:

- citation validity verifies that the model does not invent references;
- retrieval evaluation verifies whether expected evidence was found;
- human review determines whether the generated response itself remains appropriately grounded and useful.

The generated human-review report is treated as a transient evaluation artefact and remains outside source control. Stable methodology, baseline results and significant findings are recorded in this document instead.

## Evaluation Outputs

Each evaluation run produces two complementary outputs.

### Machine-Readable Evaluation Report

A timestamped JSON report records:
...
A timestamped Markdown report records:

- evaluation inputs;
- expected evidence;
- actual generated AI responses;
- automated grounding results;
- structured human-review criteria;
- reviewer notes.

Generated reports are written beneath the evaluation project's build output and are not committed to source control. They are execution artefacts rather than permanent benchmark documents.

Significant baseline results and findings are instead recorded in this document so the repository retains a stable evaluation history.
